// tvtesuexit.cpp : アプリケーションのエントリ ポイントを定義します。
//

#include "stdafx.h"
#include "tvtesuexit.h"
                     ...
                     _In_ int       nCmdShow)
{
	...
	while (true)
	{
		...
		std::wstring filen = L"TVTest.exe";

		auto bResult = Process32First(hSnap, &pe);					//最初のプロセスの情報をpeに格納
		while (bResult)//ヌルになるまで繰り返し
		{
			auto nn = filen.find(pe.szExeFile);
    ...
    HACCEL hAccelTable = LoadAccelerators(hInstance, MAKEINTRESOURCE(IDC_TVTESUEXIT));
...
ATOM MyRegisterClass(HINSTANCE hInstance)
{
    ...
    wcex.hIcon          = LoadIcon(hInstance, MAKEINTRESOURCE(IDI_TVTESUEXIT));
    wcex.hCursor        = LoadCursor(nullptr, IDC_ARROW);
    wcex.hbrBackground  = (HBRUSH)(COLOR_WINDOW+1);
    wcex.lpszMenuName   = MAKEINTRESOURCEW(IDC_TVTESUEXIT);
